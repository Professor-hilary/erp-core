...
impl<R: AccountRepository, T: TransactionRepository> TransactionService<R, T> {
    ...
    pub async fn create_transaction(&self, user_id: Uuid, tx: &CreateTransaction) -> Result<Transaction, AppError> {
        ...
        self.account_repo.update_balance(tx.debit_account_id, -tx.amount).await?;
        self.account_repo.update_balance(tx.credit_account_id, tx.amount).await?;

        Ok(transaction)
    }

    pub async fn get_transaction(&self, id: Uuid, user_id: Uuid) -> Result<Transaction, AppError> {
        self.tx_repo.find_by_id(id, user_id).await?
    ...
    pub async fn update_transaction(&self, id: Uuid, user_id: Uuid, tx: &CreateTransaction) -> Result<Transaction, AppError> {
        // Get old transaction to reverse balances
        let old = self.tx_repo.find_by_id(id, user_id).await?
            .ok_or(AppError::NotFound("Transaction not found".into()))?;

        // Validate new accounts
        let _debit_acc = self.account_repo.find_by_id(tx.debit_account_id, user_id).await?
            .ok_or(AppError::NotFound("New debit account not found".into()))?;
        let _credit_acc = self.account_repo.find_by_id(tx.credit_account_id, user_id).await?
            .ok_or(AppError::NotFound("New credit account not found".into()))?;

        // Reverse old balances
        self.account_repo.update_balance(old.debit_account_id, old.amount).await?;
        self.account_repo.update_balance(old.credit_account_id, -old.amount).await?;
    ...
    pub async fn delete_transaction(&self, id: Uuid, user_id: Uuid) -> Result<(), AppError> {
        let tx = self.tx_repo.find_by_id(id, user_id).await?
            .ok_or(AppError::NotFound("Transaction not found".into()))?;
