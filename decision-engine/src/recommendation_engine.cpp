...
public:
    ...
    virtual Recommendation execute(
        ...
        const std::map<std::string, std::string>& cat_feats
    ) = 0;
};

// dlib-based ML strategy (Random Forest regression → score → map to action)
class DlibRFStrategy : public RecommendationStrategy {
    dlib::random_forest_regression_function<dlib::dense_feature_extractor> model;
...
public:
    DlibRFStrategy() {
        ...
        spdlog::info("dlib Random Forest strategy initialized (regression scoring)");
        // Example: assume 5 features in fixed order
        feature_order = {"customer_ltv", "margin_potential", "recency_score", "stock_level", "demand_trend"};
    }
    ...
    Recommendation execute(
        ...
        // Map input to fixed vector (missing = 0 or mean imputation in real impl)
        ...
        double conf = std::clamp((raw_score + 1.0) / 2.0, 0.0, 1.0);  // dummy sigmoid-like
        ...
        // Dummy feature importance (in real: use OOB permutation importance from dlib)
        std::vector<std::pair<std::string, double>> contribs;
        for (const auto& f : feature_order) {
            contribs.emplace_back(f, 0.15 + (rand() % 30)/100.0);  // placeholder
        }

        return {action, conf, "dlib RF regression score → mapped action", contribs};
    }
};
...
public:
    ...
    Recommendation recommend(
        ...
        auto rec = strategy->execute(context_type, numeric_features, categorical_features);
...
std::unique_ptr<RecommendationEngine> RecommendationEngine::create(const std::string& path) {
